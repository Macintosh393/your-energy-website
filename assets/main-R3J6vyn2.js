var _=Object.defineProperty;var J=(t,e,i)=>e in t?_(t,e,{enumerable:!0,configurable:!0,writable:!0,value:i}):t[e]=i;var v=(t,e,i)=>J(t,typeof e!="symbol"?e+"":e,i);import{a as g,i as h}from"./vendor-XJ88ArH8.js";(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))s(a);new MutationObserver(a=>{for(const r of a)if(r.type==="childList")for(const o of r.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&s(o)}).observe(document,{childList:!0,subtree:!0});function i(a){const r={};return a.integrity&&(r.integrity=a.integrity),a.referrerPolicy&&(r.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?r.credentials="include":a.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function s(a){if(a.ep)return;a.ep=!0;const r=i(a);fetch(a.href,r)}})();const n={filter:"Muscles",filterValue:"",pagination:null};g.defaults.baseURL="https://your-energy.b.goit.study/api";async function V(){const t=await g.get(`/filters?filter=${n.filter}&page=${n.pagination.page}&limit=${n.pagination.perPage}`);return n.pagination.page===1&&n.pagination.setMaxPage(t.data.totalPages),t.data.results}async function U(t=""){const e=new URLSearchParams({page:n.pagination.page,limit:n.pagination.perPage});switch(t!=""&&e.set("keyword",t),n.filter){case"Muscles":e.set("muscles",n.filterValue);break;case"Body parts":e.set("bodypart",n.filterValue);break;case"Equipment":e.set("equipment",n.filterValue)}const i=await g.get(`/exercises?${e}`);return n.pagination.page===1&&n.pagination.setMaxPage(i.data.totalPages),i.data.results}async function T(t){return(await g.get(`/exercises/${t}`)).data}async function j(){return(await g.get("/quote")).data}async function Q(t){try{await g.post("/subscription",{email:t}),h.success({title:"Success",message:"You are now subscribed",position:"bottomCenter"})}catch{h.error({title:"Error",message:"Couldn't subscribe. Email might be already registered",position:"bottomCenter"})}}async function W(t,e){await g.patch(`/exercises/${t}/rating`,e)}function c(t){return t.charAt(0).toUpperCase()+t.slice(1)}const w=document.querySelector(".pagination-pages"),b=document.querySelector(".pagination-back-wrapper"),x=document.querySelector(".pagination-next-wrapper");function d(t,e){e===1?(w.style.visibility="hidden",b.style.visibility="hidden",x.style.visibility="hidden"):(w.style.visibility="visible",b.style.visibility="visible",x.style.visibility="visible"),t===1?b.style.opacity="20%":b.style.opacity="100%",t===e?x.style.opacity="20%":x.style.opacity="100%";let i=[];for(let a=t-2;a<=t+2;a++)a>0&&a<=e&&i.push(a);let s=i.map(a=>`<li class="pagination-page-number ${a==t?"current":""}">${a}</li>`).join("");t<e-2&&(s+='<li class="pagination-continues">...</li>'),w.innerHTML=s}const G=document.querySelector(".content-list");async function l(t=""){const e=(await U(t)).map(i=>`
      ...
      </li>`).join("");G.innerHTML=e,d(n.pagination.page,n.pagination.maxPage)}let S;K();function f(){return JSON.parse(localStorage.getItem("favorites"))??[]}async function Y(t){const e=f(),i=JSON.stringify([...e,t]);localStorage.setItem("favorites",i),X(t)}function F(t){const e=f(),i=JSON.stringify(e.filter(s=>s!=t));localStorage.setItem("favorites",i),ee(t),n.pagination.purpose==="favorites"&&(n.pagination.updateSize(f().length),u())}function Z(t){return S.has(t)}function K(){S=new Set(f())}function X(t){S.add(t)}function ee(t){S.delete(t)}const P=document.querySelector(".favorite-exercises-list"),te=document.querySelector(".pagination");async function u(){const t=f();if(t.length===0){P.innerHTML=`
    <li class="favorites-placeholder">
        ...
        like to your favorites for easier access in the future.
    </li>`,te.style.display="none";return}const e=(n.pagination.page-1)*n.pagination.perPage,i=n.pagination.page*n.pagination.perPage,s=t.slice(e,i),a=await Promise.all(s.map(async r=>{const o=await T(r);return`
          <li class="exercise">
            <div class="exercise-line-wrapper">
                <span class="workout">workout</span>
                <div class="rating">
          ...
          </li>`}));P.innerHTML=a.join("")}const ie=document.querySelector(".content-list");async function p(){const t=(await V()).map(e=>`<li class="filter-tile" data-value="${e.name}" aria-label="${e.name}">
    ...
    `}const M=document.querySelector(".modal-backdrop"),be=document.querySelector(".modal-close-btn"),q=document.querySelector(".modal-body"),xe=document.querySelector("header"),qe=document.querySelector("body");function D(){const t=function(){const i=document.querySelector("#rating-form");i?k(i.dataset.id):(M.classList.add("is-hidden"),qe.classList.remove("modal-open"),xe.style.visibility="visible")},e=function(i){i.key==="Escape"&&t()};M.addEventListener("click",t),document.addEventListener("keydown",e),be.addEventListener("click",t),q.addEventListener("click",i=>{i.stopPropagation()}),q.addEventListener("click",i=>{const s=i.target.closest(".give-rating-btn");if(!s)return;const a=s.dataset.id;ye(a)}),q.addEventListener("submit",i=>{try{i.preventDefault();const s=i.target.closest("#rating-form");if(!s)return;const a=Number(s.querySelector(".rating-form-value").innerHTML),r=s.querySelector("#rating-email-input").value,o=s.querySelector("#rating-review-text").value,y={rate:a,email:r,review:o};if(!O(r))throw new Error("Invalid email");if(a==0)throw new Error("Rating can't be 0");const $=s.dataset.id;W($,y),k($),h.success({title:"Success",message:"Posted a review",position:"bottomCenter"})}catch(s){h.error({title:"Error",message:s.message,position:"bottomCenter"})}}),q.addEventListener("change",i=>{if(i.target.type==="radio"){const s=document.querySelector(".rating-form-value");s.innerHTML=i.target.value}})}const Se=document.querySelector(".modal-body");function z(){Se.addEventListener("click",t=>{const e=t.target.closest(".add-to-favorites-btn"),i=t.target.closest(".remove-from-favorites-btn");if(e){const s=e.dataset.id;Y(s);const a=`
